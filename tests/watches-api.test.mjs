...
test("watches endpoint returns one compact catalogue and caches the Airtable read", async () => {
  ...
  try {
    ...
    const stale = await onRequest(staleContext);
    ...
    assert.equal(stale.headers.get("X-Stapleford-Cache"), "STALE");
    assert.equal(stalePayload.watches[0].price, 10001);
    assert.equal(refreshed.headers.get("X-Stapleford-Cache"), "EDGE");
