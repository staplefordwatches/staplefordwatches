...
function addAddressCollection(params, env) {
  allowedShippingCountries(env).forEach((country, index) => {
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const requestedId = body.id || body.watch || (Array.isArray(body.items) && body.items[0] && body.items[0].id);
    const watch = await getAvailableWatch(env, requestedId);

    const params = new URLSearchParams();
    const origin = siteUrl(env, request);
    const dummyRate = {
      displayName: "Delivery calculated at checkout",
      amount: 0,
      minDays: 1,
      maxDays: 7
    };
    ...
    params.set("return_url", `${origin}/success.html?session_id={CHECKOUT_SESSION_ID}`);
    params.set("billing_address_collection", "required");
    params.set("phone_number_collection[enabled]", "true");
    params.set("consent_collection[terms_of_service]", "required");
    params.set("client_reference_id", watch.listingId);
    ...
    params.set("metadata[source]", "embedded_checkout");

    if (String(env.STRIPE_AUTOMATIC_TAX || "").toLowerCase() === "true") {
      params.set("automatic_tax[enabled]", "true");
    }

    addAddressCollection(params, env);
    appendShippingOption(params, 0, dummyRate);

    params.set("line_items[0][quantity]", "1");
    params.set("line_items[0][price_data][currency]", "gbp");
    params.set("line_items[0][price_data][unit_amount]", String(Math.round(watch.price * 100)));
    params.set("line_items[0][price_data][product_data][name]", watch.name);
    params.set("line_items[0][price_data][product_data][metadata][listing_id]", watch.listingId);
    params.set("line_items[0][price_data][product_data][metadata][airtable_record_id]", watch.airtableRecordId);
